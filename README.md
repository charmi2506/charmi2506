<p align="center">

<img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white"/>

<img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react"/>

<img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js"/>

<img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb"/>

<img src="https://img.shields.io/badge/AI-Gemini-blue?style=for-the-badge"/>

</p>
<h1 align="center">Hi 👋, I'm Charmi Gubbala</h1>
<h3 align="center">AI & Full Stack Developer | Problem Solver | B.Tech CSE Student</h3>

<p align="center">
Passionate about building AI-powered and scalable web applications while solving real-world problems through technology.
</p>

---

## 👩‍💻 About Me

- 🎓 B.Tech Computer Science Engineering
- 💻 Solved **650+ DSA Problems**
- 🚀 Interested in **Full Stack Development & Artificial Intelligence**
- 🌱 Currently learning **System Design & Advanced Backend Development**
- 📍 Andhra Pradesh, India

---

## 🛠 Tech Stack

### Languages
- C++
- Java
- Python
- JavaScript
- SQL

### Frontend
- React.js
- HTML5
- CSS3
- Tailwind CSS
- Bootstrap

### Backend
- Node.js
- Express.js

### Database
- MongoDB
- MySQL

### AI & Tools
- Gemini API
- Git
- GitHub
- VS Code
- Postman

---

# 🚀 Featured Projects

## 🔹 PolicyGuard AI
AI-powered Privacy Policy Analyzer that detects risky clauses, summarizes policies and provides a Privacy Risk Score.

**Tech Stack**
- React
- Node.js
- Express
- MongoDB
- Gemini API

---

## 🔹 Expense Tracker
JWT authenticated Full Stack Expense Management System with CRUD operations and analytics.

**Tech Stack**
- React
- Node.js
- Express
- MongoDB

---

## 🔹 WebRTC Video Chat
Real-time Video Calling application supporting peer-to-peer communication.

**Tech Stack**
- Next.js
- WebRTC
- Socket.io

---

## 🔹 AI Chrome Extension
Summarizes webpages instantly using AI.

---

# 📈 Coding Profiles

- 💻 LeetCode
- 🟢 GeeksforGeeks
- 🔵 CodeChef

Solved **650+ DSA Problems**

---

# 📊 GitHub Stats

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=charmi2506&show_icons=true&theme=tokyonight"/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=charmi2506&theme=tokyonight"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=charmi2506&layout=compact&theme=tokyonight"/>

</p>

---

# 📫 Connect With Me

- 📧 Email: 23A91A6181@aec.edu.in
- 💼 LinkedIn: https://www.linkedin.com/in/charmi-gubbala-3bb927290/

---

⭐ Thanks for visiting my profile!
